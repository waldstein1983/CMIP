...
void initOriginal() {
    for (int i = 1; i <= numFacility; i++) {
        y[i] = XPRBnewvar(masterSolver, XPRB_BV, XPRBnewname("y_%d", i), 0, 1);
    }

    for (int i = 1; i <= numFacility; i++) {
        for (int j = 1; j <= numCustomer; j++) {
            x[i][j] = XPRBnewvar(masterSolver, XPRB_PL, XPRBnewname("x_%d_%d", i, j), 0, MAX);
        }
...
//    XPRBprintctr(totalCapacity);
    ...
    for (int j = 1; j <= numCustomer; j++) {
        XPRBctr demandConserv = XPRBnewctr(masterSolver, XPRBnewname("demand_%d", j), XPRB_E);
        for (int i = 1; i <= numFacility; i++) {
...
//        XPRBprintctr(demandConserv);
    ...
    for (int i = 1; i <= numFacility; i++) {
        for (int j = 1; j <= numCustomer; j++) {
            XPRBctr ctr = XPRBnewctr(masterSolver, XPRBnewname("Bouding %d %d ", i, j), XPRB_L);
...
//            boundingCtrs[i][j] = ctr;
    ...
    for (int i = 1; i <= numFacility; i++) {
        XPRBctr ctr = XPRBnewctr(masterSolver, XPRBnewname("Capacity %d", i), XPRB_L);
        for (int j = 1; j <= numCustomer; j++) {
...
void solveOriginalModel() {
    ...
    cout << "Optimal -> " << XPRBgetobjval(masterSolver) << endl;
//    printMasterVar();
//    printSubVar();
}
    ...
    while (getline(in, str)) {
        ...
        if (lineId == 1) {
            ...
            copy(istream_iterator<string>(iss), istream_iterator<string>(),
                 back_inserter(tokens));
            if (tokens.size() == 2) {
                openingCosts[lineId - 1] = stoi(tokens[1]);
                capacities[lineId - 1] = stoi(tokens[0]);
            } else if (tokens.size() == numCustomer) {
                if (lineId == numFacility + 1 + 1) {
                    for (int id = 0; id < tokens.size(); id++) {
                        demands[id + 1] = stoi(tokens[id]);
                    }
                } else {
                    for (int id = 0; id < tokens.size(); id++) {
...
//    string fileName = "/home/local/ANT/baohuaw/CLionProjects/CMIP/data/ufl/KoerkelGhosh-sym/250/b/gs250b-3";
    string fileName = "/home/local/ANT/baohuaw/CLionProjects/CMIP/data/cfl/Beasley/cap61";
